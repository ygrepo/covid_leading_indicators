...
class generalised_additive_regression:
    ...
    def backward_selection(self, X, y, max_dfs, max_dfs_model=None):
        ...
        self.ols, self.dfs, self.X_train_gam, self.cv_scores = GAM_backward_selection(self.X_train, self.y_train, dfs,
                                                                                      max_dfs_model)
...
def plot_var(df, col_name, full_name, continuous):
    ...
    if continuous:
        sns.boxplot(x='Channel', y=col_name, data=df, ax=ax2)
        ax2.set_ylabel('')
        ax2.set_title(full_name + ' by Channel')
    else:
        Charged_Off_rates = df.groupby(col_name)['Channel'].value_counts(normalize=True)[:, 1]
    ...
    if continuous:
        facet = sns.FacetGrid(df, hue='Channel', size=3, aspect=4)
        facet.map(sns.kdeplot, col_name, shade=True)
        # facet.set(xlim=(df[col_name].min(), df[col_name].max()))
        facet.add_legend()
    else:
        fig = plt.figure(figsize=(12, 3))
        sns.countplot(x=col_name, hue='Channel', data=df, order=sorted(df[col_name].unique()))
